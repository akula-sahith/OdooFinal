...
import { CUSTOMER_QUOTATION_STATUS, CUSTOMER_STATUS_LABELS } from '../types/customerQuotationTypes';
      ...
      <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search by Quotation Number, Title, or Request ID..."
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {Object.keys(CUSTOMER_QUOTATION_STATUS).map((statusKey) => (
          ...
          {(filters.search || (filters.status && filters.status !== 'ALL')) && (
            <button
              onClick={onReset}
              disabled={loading}
              className="px-3 py-2 text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-700/50 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg flex items-center gap-1.5 transition"
            >
