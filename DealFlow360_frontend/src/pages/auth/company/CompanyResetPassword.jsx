...
export const CompanyResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await resetPassword(token, password, 'company');
      setIsSuccess(true);
    } catch (err) {
      setErrorMessage(
  ...
  return (
    <AuthLayout portal="company" title="Set New Password">
      {!isSuccess ? (
        ...
        <div className="text-center space-y-5">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-slate-600 font-medium">
              Your staff password has been updated. You can now log in.
            </p>
          </div>

          <PrimaryButton
            portal="company"
            onClick={() => navigate('/m-entry-z7829a/login')}
            icon={<ArrowRight className="w-4 h-4" />}
          >
            Continue to Company Sign In
          </PrimaryButton>
        </div>
      )}
    </AuthLayout>
